...
func TestGenerateReceiptPDF(t *testing.T) {
	...
	err = ioutil.WriteFile("../../scratch/test_hotel_receipt.pdf", pdfBytes, 0644)
	if err != nil {
		t.Fatalf("failed to save hotel fee pdf: %v", err)
	}
	fmt.Println("SUCCESS: Saved scratch/test_hotel_receipt.pdf")

	// Test Tobacco Tax (Cheque payment copy)
	...
	err = ioutil.WriteFile("../../scratch/test_tobacco_receipt.pdf", pdfBytes2, 0644)
	if err != nil {
	...
	err = ioutil.WriteFile("../../scratch/test_oil_gas_receipt.pdf", pdfBytes3, 0644)
	if err != nil {
